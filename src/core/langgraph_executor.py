...
class LangGraphExecutor:
    ...
    def _check_if_anything_to_do(self):
        ...
        """
        # 取得記憶器官
        memory_organ = self.organs.get("memory")
        if memory_organ and hasattr(memory_organ, "get_all_facts"):
            try:
                facts = memory_organ.get_all_facts()  # 取得所有記憶
                # 檢查記憶中是否有未完成的任務
                for fact in facts:
    ...
    def _build_tools(self) -> list:
        ...
        if memory_organ:
            def remember_fact(fact: str) -> str:
                """記住一個事實"""
                if hasattr(memory_organ, "remember_fact"):
                    memory_organ.remember_fact(fact, importance=0.7)
            ...
            def recall_memory(query: str) -> str:
                """回憶相關記憶"""
                if hasattr(memory_organ, "recall"):
                    results = memory_organ.recall(query)
                    if results:
                        return "\n".join(results[:5])
                    return "📭 沒有相關記憶"
                return "⚠️ 記憶模組不可用"
    ...
    def _self_reflect(self, user_msg: str, reply: str, depth: int = 0) -> str:
        ...
        try:
            ...
            if "✅" in result:
                ...
                # 記錄錯誤到記憶
                memory_organ = self.organs.get("memory")
                if memory_organ and hasattr(memory_organ, "remember_fact"):
                    memory_organ.remember_fact(
    ...
    def process(self, user_msg: str) -> str:
        ...
        if long_term_memory:
            print(f"[LangGraphExecutor] 讀取到長期記憶: {long_term_memory[:200]}")
        
        # ===== 修復 12：對話前自動回顧最近 5 次對話 =====
        recent_conversations = self._recall_recent_conversations()
        if recent_conversations:
            print(f"[LangGraphExecutor] 回顧最近對話:\n{recent_conversations[:200]}")
        
        # ===== 被動觸發 1：對話前自動記憶 =====
        # 每次收到使用者訊息時，先調用 memory.recall 查詢相關記憶
        memory_organ = self.organs.get("memory")  # 取得記憶器官
        recalled_context = ""  # 回憶內容
        if memory_organ and hasattr(memory_organ, "recall"):
            try:
                recalled_context = memory_organ.recall(user_msg)  # 呼叫 recall
                if recalled_context:
                    print(f"[LangGraphExecutor] 回憶相關記憶: {recalled_context[:200]}")
            except Exception as e:
                print(f"[LangGraphExecutor] 回憶失敗: {e}")
        
        # 將回憶內容加入使用者訊息
        enriched_msg = user_msg
        if recalled_context:
            enriched_msg = f"相關記憶：{recalled_context}\n\n使用者說：{user_msg}"
        ...
        if self.agent is not None:
            try:
                ...
                # 修復 7：把長期記憶和進化循環結果塞進 system prompt
                evolution_memory = ""
                evolution_organ = self.organs.get("evolution")
        ...
        # ===== 被動觸發 2：對話後自動記憶 =====
        # 每次回覆後，調用 memory.remember 記住對話
        if memory_organ and hasattr(memory_organ, "remember_conversation"):
            try:
                memory_organ.remember_conversation(
                    user_msg=user_msg[:200],
                    assistant_msg=agent_result[:200] if agent_result else "（無回覆）",
                    importance=0.5
                )
                print(f"[LangGraphExecutor] 已記憶對話內容")
            except Exception as e:
                print(f"[LangGraphExecutor] 記憶失敗: {e}")
        
        # ===== 修復 2：記憶自動寫入（每次對話後都執行） =====
        if agent_result:
